...
<body>
  ...
  <script src="/static/dashboard.js?v=20260508b"></script>
</body>
</html>
"""
...
function loadSample(){const samples=[{season:1,mnth:1,hr:8,weekday:1,holiday:0,workingday:1,weathersit:2,temp_c:7,hum_pct:72,wind_kmh:14},{season:2,mnth:6,hr:17,weekday:3,holiday:0,workingday:1,weathersit:1,temp_c:28,hum_pct:44,wind_kmh:9},{season:3,mnth:9,hr:13,weekday:5,holiday:0,workingday:0,weathersit:1,temp_c:24,hum_pct:38,wind_kmh:12},{season:4,mnth:12,hr:20,weekday:0,holiday:1,workingday:0,weathersit:3,temp_c:4,hum_pct:81,wind_kmh:22},{season:2,mnth:7,hr:6,weekday:2,holiday:0,workingday:1,weathersit:2,temp_c:19,hum_pct:63,wind_kmh:18}];const s=samples[Math.floor(Math.random()*samples.length)];byId("season").value=s.season;byId("mnth").value=s.mnth;byId("hr").value=s.hr;byId("weekday").value=s.weekday;byId("holiday").checked=!!s.holiday;byId("workingday").checked=!!s.workingday;byId("weathersit").value=s.weathersit;byId("hum_pct").value=s.hum_pct;byId("temp_c").value=s.temp_c;byId("wind_kmh").value=s.wind_kmh;byId("hum_pct_v").textContent=`${s.hum_pct}%`;byId("temp_c_v").textContent=`${s.temp_c} °C`;byId("wind_kmh_v").textContent=`${s.wind_kmh} km/h`}
function loadSampleBatch(){const mk=()=>{const ri=(a,b)=>Math.floor(Math.random()*(b-a+1))+a,rb=(p)=>Math.random()<p;const season=ri(1,4),mnth=ri(1,12),hr=ri(0,23),weekday=ri(0,6),workingday=rb(0.72)?1:0,holiday=workingday?0:(rb(0.18)?1:0),weathersit=rb(0.65)?1:(rb(0.75)?2:3),temp_c=ri(5,33),atemp_c=Math.max(ATEMP_MIN_C,Math.min(ATEMP_MAX_C,temp_c+ri(-2,4))),hum_pct=ri(25,90),wind_kmh=ri(0,40);return{season,mnth,hr,holiday,weekday,workingday,weathersit,temp:(temp_c-TEMP_MIN_C)/(TEMP_MAX_C-TEMP_MIN_C),atemp:(atemp_c-ATEMP_MIN_C)/(ATEMP_MAX_C-ATEMP_MIN_C),hum:hum_pct/100,windspeed:wind_kmh/WIND_MAX_KMH}};const count=3+Math.floor(Math.random()*3),items=Array.from({length:count},()=>mk());byId("batchInput").value=items.map(v=>JSON.stringify(v)).join("\\n");byId("batchCounter").textContent=`${items.length} / 100 lines`}
